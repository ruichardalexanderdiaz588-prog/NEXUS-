...
import { ArrowLeft, Video, Ghost } from "lucide-react";
...
type VideoPhase = "age" | "orientation" | "searching" | "no-results";
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute bottom-16 text-center text-white z-10">
        <h2 className="text-2xl font-bold">Buscando conexiones visuales...</h2>
        <p className="text-white/80">Prepárate para conocer nuevas almas cara a cara.</p>
      </div>
    </div>
  );
};
...
export default function VideoChatPage() {
  const [phase, setPhase] = useState<VideoPhase>("age");
    ...
    switch (phase) {
      ...
      case "orientation":
        return (
          ...
          >
            <Video className="w-16 h-16 text-primary mb-4 text-glow-primary" />
            <h2 className="text-2xl font-bold text-foreground mb-2 text-center">¡Conéctate cara a cara con el universo!</h2>
            <p className="text-muted-foreground mb-6 text-center">Descubre nuevas almas a través de la videollamada.</p>
  ...
  return (
    <div className="w-full min-h-svh flex flex-col bg-background text-foreground">
      <header className={cn("p-4 border-b border-border flex items-center sticky top-0 bg-background/80 backdrop-blur-sm z-10", phase === 'searching' && 'bg-transparent border-transparent')}>
        ...
        <h1 className={cn("text-xl font-bold text-primary ml-4", phase === 'searching' && 'text-white/80')}>Conoce por Video</h1>
      </header>
