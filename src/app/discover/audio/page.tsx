...
import { ArrowLeft, Mic, User, UserCheck, XCircle, EarOff } from "lucide-react";
...
type AudioPhase = "age" | "orientation" | "searching" | "found" | "chatting" | "no-results";
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute bottom-16 text-center text-white z-10">
        <h2 className="text-2xl font-bold">Sintonizando frecuencias de audio...</h2>
        <p className="text-white/80">Buscando almas que compartan tu vibración sonora.</p>
      </div>
    </div>
  );
};
...
export default function AudioChatPage() {
  const [phase, setPhase] = useState<AudioPhase>("age");
  ...
  useEffect(() => {
    ...
    if (phase === "searching") {
      timer = setTimeout(() => {
        // Simulate no results found for now
        setPhase("no-results");
      }, 5000);
    }
    ...
    switch (phase) {
      ...
      case "orientation":
        return (
          ...
          >
            <Mic className="text-5xl text-primary mb-4 text-glow-primary" />
            <h2 className="text-2xl font-bold text-foreground mb-2">¡Envía tu voz al universo!</h2>
            <p className="text-muted-foreground mb-6">Conecta con almas a través del poder del sonido.</p>
      ...
      case "no-results":
        return (
           <motion.div key="no-results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center p-4">
              <EarOff className="w-24 h-24 text-accent mb-6" />
              <h2 className="text-3xl font-bold text-primary mb-2">¡Ups... el éter está en silencio!</h2>
              <p className="text-muted-foreground max-w-md mb-6">
                  No hemos encontrado ninguna alma resonando por audio en este momento. Intenta con otros filtros, o regresa más tarde.
              </p>
  ...
  return (
    <div className="w-full min-h-svh flex flex-col bg-background text-foreground">
      <header className={cn("p-4 border-b border-border flex items-center sticky top-0 bg-background/80 backdrop-blur-sm z-10", phase === 'searching' && 'bg-transparent border-transparent')}>
        ...
        <h1 className={cn("text-xl font-bold text-primary ml-4", phase === 'searching' && 'text-white/80')}>Conoce por Audio</h1>
      </header>
      <main className="flex-1 flex items-center justify-center text-center relative">
        <AnimatePresence mode="wait">
          {renderContent()}
        </AnimatePresence>
      </main>
